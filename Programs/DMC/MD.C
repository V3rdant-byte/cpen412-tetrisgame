...
**	speech registers
*********************************************************************************************/
#define SPEECH_phone_Addr         *(volatile unsigned char *)(0x00701000)
#define SPEECH_busy_Addr          *(volatile unsigned char *)(0x00701001)
...
void say_phoneme   (char phoneme_code) {
    ...
    char busy = 0;
    char busy_status = 0;
    SPEECH_phone_Addr = phoneme_code;
    do {
        busy_status = SPEECH_busy_Addr;
        busy = SPEECH_busy_Addr;
        // printf("busy: %x\n", busy);
    } while (busy == 1);

    // SPEECH_phone_Addr = ((short int)(0x01<< 8) | (short int)phoneme_code);
    // SPEECH_phone_Addr = ((short int)(0x0FF) & (short int)phoneme_code);
	// while(((short int)0x01 & SPEECH_busy_Addr) == 0x01)
	// {
...
void menu(void)
{
    ...
    while(1)    {
        ...
        // if ( c == (char){'0'}) {
        //     printf("cursor to top left corner\n");
        //     go_to_top_corner();
        //     continue;
        // }
            UnknownCommand() ;
